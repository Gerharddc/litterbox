...
N.B. Litterbox is free software that does not come with any warranty or guarantees. It is not an anti-malware solution and does guarantee that your system will be protected from any attacks. Its goal is just to be BETTER THAN NOTHING but even that is not guaranteed. By using this software you agree that you are doing so at your own risk and take full responsibility for anything that might happen while using it.

## Comparison to alternatives

### Full Virtual Machine (VM)

Even though high isolation can easily be achieved using a virtual machine, the idea with Litterbox is to provide decent isolation coupled with much more convenience and much less overhead than a full VM requires. Litterbox runs everything on top of your host Linux kernel (thereby greatly reducing overhead) and inside a folder that exists directly on the host (thereby making it very simple to share files). Furthermore, Litterbox allows applications to connect directly to the Wayland server on your host system which means that applications running inside the Litterbox are graphically composed just like normal applications and seamlessly have access to things like your clipboard (so be careful what you put in there).
...
The isolation/sandboxing provided by Litterbox is not perfect and still leaves many holes and vulnerabilities. It is not intented to shield you from software that is known to be malicious so please do not run malware inside it deliberatly. Its only goal is to try and provide some damange limitation in the event that 3rd party software and/or code that you trust were to get compromised.
...
- Litterbox uses Podman as its container runtime. Thus, anything running inside a Litterbox could still exploit vulnerabilities in your Podman engine to gain full access to your system.
- Litterbox does not provide ANY networking isolation. Anything running inside a Litterbox has fully access to your host's network (including localhost) in the same way a normal application running on your system would. You should therefore be very careful to not have anything sensitive and/or vulnerable accessible on your network.
